...
def build_baac_dataframe(selected_BAAC_table: pd.DataFrame) -> pd.DataFrame:
    ...
    for year, g in selected_BAAC_table.groupby("year"):
        ...
        for url in g["url"]:
            ...
            # FIX: 2022 "carcteristiques" uses Accident_Id instead of Num_Acc
            if "Num_Acc" not in d.columns and "Accident_Id" in d.columns:
                d = d.rename(columns={"Accident_Id": "Num_Acc"})
